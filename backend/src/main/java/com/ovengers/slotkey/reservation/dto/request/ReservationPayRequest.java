 ...
 * 비교 기준이 가격 값이 아니라 version인 이유는 ABA 문제 때문이다(core-domain-decisions 5-2).
 * Idempotency-Key는 바디가 아니라 헤더로 받는다(api-명세서.md 1-7).
 */
@Schema(description = "크레딧 결제 요청. Idempotency-Key는 요청 헤더로 전달합니다.")
public record ReservationPayRequest(

        @Schema(
                description = "예약 생성 응답의 spaceVersion을 그대로 전달합니다.",
                example = "0"
        )
