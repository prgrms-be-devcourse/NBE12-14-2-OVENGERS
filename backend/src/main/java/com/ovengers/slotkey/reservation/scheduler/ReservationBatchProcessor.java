 ...
 *
 * 모든 메서드는 "조건부 UPDATE 영향 행이 1일 때만 후속 처리 + 이력 저장"(core-domain-decisions 3-1) 패턴을 따르며,
...
public class ReservationBatchProcessor {
    ...
    /** 최초 체크인 허용 구간 = NO_SHOW 판정 기준(core-domain-decisions 6-3, core-domain-decisions 8-1). 두 개의 규칙이 아니라 하나의 값이다. */
    ...
    /**
     * IN_USE -> COMPLETED(자동 퇴실, core-domain-decisions 3-4). 슬롯 반환 없음, 환불 없음(core-domain-decisions 8-4).
