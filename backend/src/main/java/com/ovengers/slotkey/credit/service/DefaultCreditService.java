 ...
 * 기본 구현체. 실제로 결제 확인/연장/취소 요청이 들어오면 즉시 실패시킨다 — 돈이 움직이는
...
public class DefaultCreditService implements CreditService {
    ...
    @Override
    public int refund(Long memberId, Long reservationId, int amount) {
        throw new IllegalStateException(NOT_IMPLEMENTED_MESSAGE);
    }

    @Override
    public int penalize(Long memberId, Long reservationId, int amount) {
        throw new IllegalStateException(NOT_IMPLEMENTED_MESSAGE);
    }
}
