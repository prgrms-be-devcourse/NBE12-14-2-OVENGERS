...
- `GET /reservations?status=&page=&size=` (인증 필요, 본인 예약만). `status`는 `HELD`/`EXPIRED`/`CONFIRMED`/`IN_USE`/`COMPLETED`/`CANCELLED`/`NO_SHOW` 중 하나
- `GET /reservations/{reservationId}` (본인 소유만). 오류: RESERVATION_NOT_FOUND(404), FORBIDDEN_NOT_OWNER(403)

### 5-4. 취소 — `DELETE /reservations/{reservationId}`
