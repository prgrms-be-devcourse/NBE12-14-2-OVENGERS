...
export default function SpaceSearchFilter({ onApply }: { onApply: (filters: SpaceSearchFilters) => void }) {
  ...
  return <form className={styles.filter} onSubmit={submit} aria-label="공간 검색 조건">
    ...
    <div className={styles.footer}>
      <p>시간대 검색은 날짜와 시작·종료 시간을 모두 선택해 주세요.<br />조회 이후 다른 예약이 생길 수 있으며, 실제 예약 시 가능 여부를 다시 확인합니다.</p>
      <div className={styles.actions}><Button onClick={() => { setDraft(EMPTY_SPACE_FILTERS); setError(null); onApply(EMPTY_SPACE_FILTERS); }}>초기화</Button><Button type="submit" variant="primary">검색</Button></div>
    </div>
  </form>;
}
