import type { AccessVerifyResult, IssuedDoorToken } from '../types/api';
 ...
 */
export function issueDoorToken(reservationId: number | string): Promise<IssuedDoorToken> {
  return api.post<IssuedDoorToken>(API_ROUTES.reservations.doorToken(reservationId));
}
...
export function verifyAccess({
  ...
  return api.post<AccessVerifyResult>(API_ROUTES.access.verify, { token, spaceId });
}
