...
import type { AdminMember, MemberStatus } from '../../types/api';
...
export default function AdminMemberListPage() {
  const [filter, setFilter] = useState<{ status: MemberStatus | ''; keyword: string }>({
    status: '',
    keyword: '',
  });
  const [target, setTarget] = useState<AdminMember | null>(null);
  const [creditTarget, setCreditTarget] = useState<AdminMember | null>(null);
  ...
  const grant = useAction(async ({ amount, reason }: { amount: number; reason: string }) => {
    ...
    <>
      ...
      <div className="filters">
        ...
        <Select
          ...
          value={filter.status}
          onChange={(event) => {
            setFilter({ ...filter, status: event.target.value as MemberStatus | '' });
            setPage(0);
          }}
        />
      </div>
