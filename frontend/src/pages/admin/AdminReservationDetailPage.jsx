...
import { ACCESS_REASON_LABEL, ACCESS_RESULT_META, RESERVATION_STATUS } from '../../constants/enums';
  ...
  return (
    <>
      <nav className="crumb" aria-label="현재 위치">
        <Link to={ROUTES.adminReservations}>예약 관리</Link>
        <span>›</span>
              ...
              <tbody>
                {data.accessLogs.map((log) => (
                  <tr key={log.id}>
                    ...
                    <td>{ACCESS_REASON_LABEL[log.reasonCode] ?? log.reasonCode}</td>
                  </tr>
                ))}
              </tbody>
            </table>
